  ...
  refreshDreams: async (token: string) => {
    ...
    try {
      ...
      set({
        isRefreshing: false,
        error:
          error instanceof Error ? error.message : 'Failed to refresh dreams',
      });
    }
  },

  addDream: (dream: DreamData) =>
    // When adding a single dream (e.g. after creating or updating one),
    // do not update `lastFetched`. Updating `lastFetched` here marks the
    // cache as fresh and can prevent a full list refetch on navigation
    // (which is why users sometimes only see the last dream). Keep the
    // previous `lastFetched` value so the store's `shouldRefetch()` logic
