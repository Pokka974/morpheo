...
const DreamCard = ({ dream, onPress }: DreamCardProps) => {
    ...
    return (
        <>
            <View className="flex-row h-40 bg-white rounded-2xl overflow-hidden shadow-sm my-2 mx-4">
                ...
                <View className="flex-1 p-2 justify-between gap-3">
                    ...
                    <Pressable onPress={onPress} className="flex-1">
                        ...
                        <Pressable onLongPress={() => showTooltip('Summary', dream.summary)}>
                            <Text className="text-sm mb-2 text-text" numberOfLines={4} ellipsizeMode="tail">
                                {dream.summary}
                            </Text>
                        </Pressable>
                    </Pressable>
                    <View className="flex-row items-center">
                        ...
                        <View className="flex-row ml-2">
                            {culturalEmojis.map((emoji, index) => (
                                <Text key={`${emoji}-${index}`} className="text-sm mr-1">
                                    {emoji}
                                </Text>
                            ))}
                        </View>
            ...
            {/* Tooltip Modal */}
            <Modal visible={tooltipVisible} transparent animationType="fade" onRequestClose={hideTooltip}>
                <Pressable onPress={hideTooltip} className="flex-1 bg-black/50 justify-center items-center p-4">
                    <Pressable className="bg-white p-4 rounded-lg max-w-[80%]">
                        <Text className="font-bold text-lg mb-2">{tooltipTitle}</Text>
                        <Text className="text-base text-gray-600">{tooltipContent}</Text>
                    </Pressable>
                </Pressable>
            </Modal>
        </>
