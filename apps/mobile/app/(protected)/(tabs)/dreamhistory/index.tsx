...
const DreamHistoryIndex = () => {
  ...
  const {
    dreamList,
    // Note: we'll call fetch/refresh via the store's getState() to keep a stable reference
    refreshDreams,
    isLoading,
    isRefreshing,
    error,
  ...
  // Initial fetch on mount: guarded to run only once per mount
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        if (dreamList.length === 0) {
          console.log('🚀 Dream history initial load - fetching dreams');
          setIsRendering(true);
          await useDreamListStore.getState().fetchDreams(token, false);
          if (!cancelled) setIsRendering(false);
        } else {
          console.log('🚀 Dream history initial load - using cached dreams');
        }

        hasInitiallyLoaded.current = true;
      } catch (err) {
        console.error('Error fetching dream history:', err);
        if (!cancelled) setIsRendering(false);
      }
    })();
  ...
  // Refresh when screen is focused, but debounce to avoid rapid repeated calls
  useFocusEffect(
    useCallback(() => {
      const now = Date.now();
      // ignore focus events that happen within 1s of the last one
      if (now - (lastFocusRef.current || 0) < 1000) {
        return;
      }
      ...
      (async () => {
        try {
          ...
          // Decide whether we actually need to refetch. Use the store's
          // shouldRefetch() helper or fetch when there is no cached data.
          const store = useDreamListStore.getState();
          const needRefetch =
            store.dreamList.length === 0 || store.shouldRefetch();

          if (!needRefetch) {
            console.log('Dream history focused - cache fresh, skipping fetch');
            return;
          }

          console.log('Dream history screen focused - refreshing dreams');
          setIsRendering(true);
          await store.fetchDreams(token, true);
          if (!cancelled) setIsRendering(false);
        } catch (err) {
          console.error('Error refreshing dreams on focus:', err);
          if (!cancelled) setIsRendering(false);
        }
      })();
  ...
  const renderDreamCard = useCallback(
    ({ item }: { item: any }) => (
      <DreamCard
        dream={item}
        onPress={() => {
  ...
  const isActuallyLoading =
    (isLoading && dreamList.length === 0) || isRendering;
      ...
      <GeneralLinearBackground>
        <SafeAreaView style={{ padding: 10, flex: 1 }}>
          ...
          >
            ...
            <Text
              style={{ fontFamily: 'Nunito', color: '#6b7280', marginTop: 16 }}
            >
              Loading your dreams...
            </Text>
          </View>
