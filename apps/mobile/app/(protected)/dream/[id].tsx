...
const DreamDetail = () => {
  ...
  const culturalRefs = Object.entries(
    dreamDetail?.culturalReferences || {}
  ).map(([culture, meaning]) => ({
    culture,
    meaning,
  }));
  ...
  return (
    <GeneralLinearBackground>
      ...
      {/* Go back button */}
      <Link
        className="rounded-full bg-peach/70 absolute top-20 left-6 p-2"
        href="/dreamhistory"
      >
      ...
      >
        <View className="flex-1 p-4 gap-2">
          ...
          <View className="flex-row justify-between items-center gap-2">
            <Text className="font-nunito text-2xl font-bold text-gray-800 w-[80%]">
              {dreamDetail?.title}
            </Text>
          ...
          <View className="bg-white p-4 rounded-xl mt-2 shadow-sm">
            <Text className="font-nunito text-lg text-gray-800 text-justify">
              {dreamDetail?.summary
                ? highlightText(dreamDetail.summary, tagColorMapping)
                : ''}
            </Text>
          </View>
          ...
          <View className="flex-col gap-2">
            <Text className="font-nunito text-lg font-semibold text-gray-800">
              Emotions:
            </Text>
            <View className="flex-row flex-wrap gap-2">
              {dreamDetail?.emotions.map((item, index) => (
                <PastelChip
                  key={`${item}-${index}`}
                  text={item}
          ...
          {dreamDetail?.recurringDreamAnalysis?.hasConnections && (
            <>
              ...
              {/* Connected Dreams */}
              {dreamDetail.recurringDreamAnalysis.connectedDreams.length >
                0 && (
                <View className="mb-4">
                  <Text className="font-nunito font-semibold text-gray-700 mb-2">
                    Connected Dreams:
                  </Text>
                  {dreamDetail.recurringDreamAnalysis.connectedDreams.map(
              ...
              {dreamDetail.recurringDreamAnalysis.patterns.length > 0 && (
                <View className="mb-4">
                  ...
                  <View className="flex-row flex-wrap gap-2">
                    {dreamDetail.recurringDreamAnalysis.patterns.map(
                      (pattern, index) => (
                        <PastelChip
                          key={`pattern-${index}`}
                          text={pattern}
                          bgColor={
                            getUniquePastelColors(
                              dreamDetail.recurringDreamAnalysis?.patterns
                                .length || 1
                            )[index] || '#e0e0e0'
                          }
                          size="md"
